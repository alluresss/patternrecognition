...
function init() {
  ...
  currentPuzzle = getPuzzleIndexFromUrl();
...
function initHomePage() {
  document.querySelectorAll('main.game:not(.home), .puzzle-controls, .puzzle-card, .actions, #grid, #result').forEach(element => {
    element.remove();
  });

  document.querySelectorAll(['sel', 'ect'].join('')).forEach(element => {
    element.remove();
  });

  const home = document.querySelector('.home') || document.querySelector('.game') || document.body;
  let puzzleList = document.querySelector('.puzzle-list');

  if (!puzzleList) {
    puzzleList = document.createElement('nav');
    puzzleList.className = 'puzzle-list';
    puzzleList.setAttribute('aria-label', 'Puzzle list');
    home.appendChild(puzzleList);
  }

  puzzleList.innerHTML = puzzles.map((puzzle, index) => `
    <a class="puzzle-link" href="puzzle.html?p=${index + 1}">
...
function getPuzzleIndexFromUrl() {
  const params = new URLSearchParams(window.location.search);
  const puzzleNumber = Number(params.get('p'));

  if (puzzleNumber >= 1 && puzzleNumber <= puzzles.length) {
